...
async function queryAzureWorkItemIds(orgUrl: string, project: string, auth: string, label: string): Promise<number[]> {
  ...
  if (!res.ok) {
    throw new Error(`Azure DevOps WIQL error ${res.status}: ${await res.text()}`);
  }
  const data = (await res.json()) as { workItems?: Array<{ id: number }> };
  return (data.workItems || []).map((w) => w.id).slice(0, 50);
}

function parseAzureWorkItem(item: { id: number; fields: Record<string, unknown>; _links?: { html?: { href?: string } } }): TrackerTicket {
...
export async function fetchAzureTickets(options: {
  ...
  const itemsRes = await fetch(itemsUrl, { headers: { Authorization: authHeader } });
  ...
  const itemsData = (await itemsRes.json()) as {
    value: Array<{ id: number; fields: Record<string, unknown>; _links?: { html?: { href?: string } } }>;
  };
