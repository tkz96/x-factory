...
function normalizeDiscoveryInput(input: RepositoryDiscoveryInput): void {
  ...
  const isAzure =
    input.provider === "azure" || input.provider === "azure-devops";
    ...
    (input) => {
      if (!input.provider) {
        return Promise.resolve(
          errorResponse(
            "Discovery provider is required ('azure', 'github', 'local').",
          ),
        );
      }
...
async function handleInspectRepository(req: Request): Promise<Response> {
  return withJsonBody<{ path?: string; expectedRemote?: string }>(
    req,
    ({ path: repoPath, expectedRemote }) => {
