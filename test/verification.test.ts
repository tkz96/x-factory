...
describe("Deterministic Verification Pipeline", () => {
  ...
  it("passes when tests succeed and non-empty diff exists without pollution", async () => {
    ...
    await writeFile(path.join(fixtureRepo, "feature.ts"), "export const x = 42;\n");
  ...
  it("fails when tests exit non-zero", async () => {
    ...
    const result = await runVerification(fixtureRepo, failingProject, baseline, 1);
  ...
  it("fails when pollution is detected", async () => {
    const baseline = await recordBaseline(fixtureRepo);
    await writeFile(path.join(fixtureRepo, "feature.ts"), "export const x = 42;\n");
    await writeFile(path.join(fixtureRepo, "debug.log"), "temporary debug log\n");
...
describe("buildRepairPrompt", () => {
  it("formats structured repair prompt with failure details and criteria", () => {
    const ticket: Ticket = {
      ...
      acceptanceCriteria: ["Tokens must expire after 1 hour", "Return 401 when token expired"],
    };
    ...
    const prompt = buildRepairPrompt(ticket, "Update auth.ts logic", mockVerification, 1);
    assert.ok(prompt.includes("Deterministic verification checks failed on attempt 1 of 3"));
    assert.ok(prompt.includes("Fix ONLY the issues required"));
    assert.ok(prompt.includes("FAIL: token expiry test expected 401 but got 200"));
